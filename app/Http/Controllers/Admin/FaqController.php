...
class FAQController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.faq.faq-general.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-general.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'image' => 'image|mimes:jpeg,png,jpg,gif|max:2048', // Wajib ada gambar
        ]);
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-general'), $imageName); // Simpan gambar di folder public/images/faq
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ baru: ' . $request->input('question'),
            'admin_id' => auth()->user->id,
        ]);

        return redirect()->route('faq-general.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ.
     */
    public function edit(Faq $faq)
    {
        return view('admin.faq.faq-general.edit', compact('faq'));
    }

    /**
     * Update FAQ yang sudah ada di database.
     */
    public function update(Request $request, Faq $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-general/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-general'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ: ' . $faq->question,
            'admin_id' => auth()->user->id,
        ]);

        return redirect()->route('faq-general.index')->with('success', 'FAQ berhasil diperbarui.');
    }
    ...
    public function destroy(Faq $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ: ' . $faq->question,
            'admin_id' => auth()->user->id,
        ]);

        return redirect()->route('faq-general.index')->with('success', 'FAQ berhasil dihapus.');
    }
}