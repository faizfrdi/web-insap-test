...
class CoFmModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqManufacturingCoFmModule::query();
        ...
        return view('admin.faq.faq-manufacturing.co-fm-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-manufacturing.co-fm-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-manufacturing/co-fm'), $imageName); // Simpan gambar di folder public/images/faq-manufacturing/co-fm
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ CO/FM Module Manufacturing: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.co-fm-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ CO/FM Module.
     */
    public function edit(FaqManufacturingCoFmModule $faq)
    {
        return view('admin.faq.faq-manufacturing.co-fm-module.edit', compact('faq'));
    }

    /**
     * Update FAQ CO/FM Module yang sudah ada di database.
     */
    public function update(Request $request, FaqManufacturingCoFmModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-manufacturing/co-fm/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-manufacturing/co-fm'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ CO/FM Module Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.co-fm-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ CO/FM Module dari database.
     */
    public function destroy(FaqManufacturingCoFmModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ CO/FM Module Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.co-fm-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}