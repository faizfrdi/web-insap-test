...
class PsModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqJasaNonKonstruksiPsModule::query();
        ...
        return view('admin.faq.faq-jasa-non-konstruksi.ps-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-jasa-non-konstruksi.ps-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-jasa-non-konstruksi/ps'), $imageName); // Simpan gambar di folder public/images/faq-jasa-non-konstruksi/ps
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ PS Module Jasa Non Konstruksi: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.ps-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ PS Module.
     */
    public function edit(FaqJasaNonKonstruksiPsModule $faq)
    {
        return view('admin.faq.faq-jasa-non-konstruksi.ps-module.edit', compact('faq'));
    }

    /**
     * Update FAQ PS Module yang sudah ada di database.
     */
    public function update(Request $request, FaqJasaNonKonstruksiPsModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-jasa-non-konstruksi/ps/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-jasa-non-konstruksi/ps'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ PS Module Jasa Non Konstruksi: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.ps-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ PS Module dari database.
     */
    public function destroy(FaqJasaNonKonstruksiPsModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ PS Module Jasa Non Konstruksi: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.ps-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}