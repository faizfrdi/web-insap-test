...
@section('content')
    <div id="flow-section" class="text-center p-3 p-md-5 mb-5" style="border-radius: 15px; border: 1px solid #ddd; background-color: #fbfbfd;">
        <h2 class="section-title mb-4" style="font-size: min(24px, 5vw);">FLOW END TO END ALL PROCESS SCENARIO JASA KONSTRUKSI</h2>
        ...
        <div class="d-flex flex-column flex-md-row justify-content-center align-items-center">
            <div class="image-container position-relative" style="width: 100%; max-width: 800px; margin: 0 auto; border-radius: 10px; overflow: hidden;">
                <img src="{{ asset('images/scenario-jasa-konstruksi.png') }}" class="img-fluid" alt="Flow End to End Jasa Konstruksi">
            </div>
    ...
    <div id="jasa-konstruksi-section" class="text-center p-3 p-md-5" style="border-radius: 15px; border: 1px solid #ddd; background-color: #fbfbfd;">
        ...
        <div id="jasa-konstruksi-content">
            <div class="row">
                @forelse ($jasaKonstruksis as $jasa_konstruksi)
                    @php
                        $moduleColors = [
                            'FI Module' => '#ff0404',
                            'PS Module' => '#aeeff0', 
                    ...
                    <div class="custom-col mb-4 jasa-card {{ $jasa_konstruksi->module == 'FI Module' ? 'fi-module' : '' }}" data-module="{{ $jasa_konstruksi->module }}">
                        <a href="{{ route('scenarios.jasa-konstruksi', $jasa_konstruksi->slug) }}" class="text-decoration-none">
                            <div class="card h-100 portfolio-card" style="border-radius: 10px; overflow: hidden; padding: 0; background-color: {{ $backgroundColor }}; background-size: cover; background-position: center;"> 
                                <div class="card-body" style="background: rgba(255,255,255,0.15); color: {{ $jasa_konstruksi->module == 'FI Module' ? 'white' : 'black' }};">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (scrollToSectionBtn) {
            scrollToSectionBtn.addEventListener('click', function (e) {
                e.preventDefault();
                document.getElementById('jasa-konstruksi-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            });
        }
