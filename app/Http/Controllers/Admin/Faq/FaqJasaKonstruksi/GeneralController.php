...
class GeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.faq.faq-jasa-konstruksi.general.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-jasa-konstruksi.general.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-jasa-konstruksi'), $imageName); // Simpan gambar di folder public/images/faq-jasa-konstruksi
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ General baru: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.general.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ General.
     */
    public function edit(FaqJasaKonstruksi $faq)
    {
        return view('admin.faq.faq-jasa-konstruksi.general.edit', compact('faq'));
    }

    /**
     * Update FAQ General yang sudah ada di database.
     */
    public function update(Request $request, FaqJasaKonstruksi $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-jasa-konstruksi/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-jasa-konstruksi'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ General: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.general.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ General dari database.
     */
    public function destroy(FaqJasaKonstruksi $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ General: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.general.index')->with('success', 'FAQ berhasil dihapus.');
    }
}