...
class GeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqInternalProject::query();
        ...
        return view('admin.faq.faq-internal-project.general.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-internal-project.general.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-internal-project'), $imageName); // Simpan gambar di folder public/images/faq-internal-project/general
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ General Internal Project: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.general.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ General.
     */
    public function edit(FaqInternalProject $faq)
    {
        return view('admin.faq.faq-internal-project.general.edit', compact('faq'));
    }

    /**
     * Update FAQ General yang sudah ada di database.
     */
    public function update(Request $request, FaqInternalProject $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-internal-project/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-internal-project'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ General Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.general.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ General dari database.
     */
    public function destroy(FaqInternalProject $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ General Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.general.index')->with('success', 'FAQ berhasil dihapus.');
    }
}