...
@section('content')
    <div id="flow-section" class="text-center p-3 p-md-5 mb-5" style="border-radius: 15px; border: 1px solid #ddd; background-color: #fbfbfd;">
        <h2 class="section-title mb-4" style="font-size: min(24px, 5vw);">FLOW END TO END ALL PROCESS SCENARIO CAPEX PROCUREMENT</h2>
        ...
        <div class="d-flex flex-column flex-md-row justify-content-center align-items-center">
            <div class="image-container position-relative" style="width: 100%; max-width: 800px; margin: 0 auto; border-radius: 10px; overflow: hidden;">
                <img src="{{ asset('images/scenario-capex-procurement.png') }}" class="img-fluid" alt="Flow End to End Capex Procurement" style="object-fit: contain; width: 100%; height: 150px;">
            </div>
    ...
    <div id="capex-procurement-section" class="text-center p-3 p-md-5" style="border-radius: 15px; border: 1px solid #ddd; background-color: #fbfbfd;">
        ...
        <div id="capex-procurement-content">
            <div class="row">
                @forelse ($capexProcurements as $capex_procurement)
                    @php
                        $moduleColors = [
                            'FI Module' => '#ff0404',
                            'PS Module' => '#aeeff0', 
                    ...
                    <div class="custom-col mb-4 capex-card {{ $capex_procurement->module == 'FI Module' ? 'fi-module' : '' }}" data-module="{{ $capex_procurement->module }}">
                        <a href="{{ route('scenarios.capex-procurement', $capex_procurement->slug) }}" class="text-decoration-none">
                            <div class="card h-100 portfolio-card" style="border-radius: 10px; overflow: hidden; padding: 0; background-color: {{ $backgroundColor }}; background-size: cover; background-position: center;"> 
                                <div class="card-body" style="background: rgba(255,255,255,0.15); color: {{ $capex_procurement->module == 'FI Module' ? 'white' : 'black' }};">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const cards = document.querySelectorAll('.capex-card');
        ...
        if (scrollToSectionBtn) {
            scrollToSectionBtn.addEventListener('click', function (e) {
                e.preventDefault();
                document.getElementById('capex-procurement-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            });
        }
