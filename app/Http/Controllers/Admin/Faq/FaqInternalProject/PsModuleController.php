...
class PsModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqInternalProjectPsModule::query();
        ...
        return view('admin.faq.faq-internal-project.ps-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-internal-project.ps-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-internal-project/ps'), $imageName); // Simpan gambar di folder public/images/faq-internal-project/ps
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ PS Module Internal Project: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.ps-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ PS Module.
     */
    public function edit(FaqInternalProjectPsModule $faq)
    {
        return view('admin.faq.faq-internal-project.ps-module.edit', compact('faq'));
    }

    /**
     * Update FAQ PS Module yang sudah ada di database.
     */
    public function update(Request $request, FaqInternalProjectPsModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-internal-project/ps/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-internal-project/ps'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ PS Module Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.ps-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ PS Module dari database.
     */
    public function destroy(FaqInternalProjectPsModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ PS Module Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.ps-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}