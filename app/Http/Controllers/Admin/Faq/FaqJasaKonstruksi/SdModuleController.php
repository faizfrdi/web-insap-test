...
class SdModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqJasaKonstruksiSdModule::query();
        ...
        return view('admin.faq.faq-jasa-konstruksi.sd-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-jasa-konstruksi.sd-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-jasa-konstruksi/sd'), $imageName); // Simpan gambar di folder public/images/faq-jasa-konstruksi
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ SD Module baru: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.sd-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ SD Module.
     */
    public function edit(FaqJasaKonstruksiSdModule $faq)
    {
        return view('admin.faq.faq-jasa-konstruksi.sd-module.edit', compact('faq'));
    }

    /**
     * Update FAQ SD Module yang sudah ada di database.
     */
    public function update(Request $request, FaqJasaKonstruksiSdModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-jasa-konstruksi/sd/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-jasa-konstruksi/sd'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ SD Module: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.sd-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ SD Module dari database.
     */
    public function destroy(FaqJasaKonstruksiSdModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ SD Module: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.sd-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}