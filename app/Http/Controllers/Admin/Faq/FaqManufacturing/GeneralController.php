...
use App\Models\FaqManufacturing; // Menggunakan FaqManufacturing
...
class GeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.faq.faq-manufacturing.general.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-manufacturing.general.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-manufacturing'), $imageName); // Simpan gambar di folder public/images/faq-manufacturing/general
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ General Manufacturing: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.general.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ General.
     */
    public function edit(FaqManufacturing $faq)
    {
        return view('admin.faq.faq-manufacturing.general.edit', compact('faq'));
    }

    /**
     * Update FAQ General yang sudah ada di database.
     */
    public function update(Request $request, FaqManufacturing $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-manufacturing/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-manufacturing'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ General Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.general.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ General dari database.
     */
    public function destroy(FaqManufacturing $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ General Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.general.index')->with('success', 'FAQ berhasil dihapus.');
    }
}