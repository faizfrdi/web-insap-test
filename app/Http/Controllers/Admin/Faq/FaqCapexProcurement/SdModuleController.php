...
class SdModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqCapexProcurementSdModule::query();
        ...
        return view('admin.faq.faq-capex-procurement.sd-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-capex-procurement.sd-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-capex-procurement/sd'), $imageName); // Simpan gambar di folder public/images/faq-capex-procurement/sd
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ SD Module Capex Procurement: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.sd-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ SD Module.
     */
    public function edit(FaqCapexProcurementSdModule $faq)
    {
        return view('admin.faq.faq-capex-procurement.sd-module.edit', compact('faq'));
    }

    /**
     * Update FAQ SD Module yang sudah ada di database.
     */
    public function update(Request $request, FaqCapexProcurementSdModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-capex-procurement/sd/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-capex-procurement/sd'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ SD Module Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.sd-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ SD Module dari database.
     */
    public function destroy(FaqCapexProcurementSdModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ SD Module Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.sd-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}