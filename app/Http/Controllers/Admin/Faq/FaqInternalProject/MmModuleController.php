...
class MmModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqInternalProjectMmModule::query();
        ...
        return view('admin.faq.faq-internal-project.mm-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-internal-project.mm-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-internal-project/mm'), $imageName); // Simpan gambar di folder public/images/faq-internal-project/mm
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ MM Module Internal Project: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.mm-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ MM Module.
     */
    public function edit(FaqInternalProjectMmModule $faq)
    {
        return view('admin.faq.faq-internal-project.mm-module.edit', compact('faq'));
    }

    /**
     * Update FAQ MM Module yang sudah ada di database.
     */
    public function update(Request $request, FaqInternalProjectMmModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-internal-project/mm/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-internal-project/mm'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ MM Module Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.mm-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ MM Module dari database.
     */
    public function destroy(FaqInternalProjectMmModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ MM Module Internal Project: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-internal-project.mm-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}