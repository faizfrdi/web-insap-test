...
class MmModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqJasaKonstruksiMmModule::query();
        ...
        return view('admin.faq.faq-jasa-konstruksi.mm-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-jasa-konstruksi.mm-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-jasa-konstruksi/mm'), $imageName); // Simpan gambar di folder public/images/faq-jasa-konstruksi
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ MM Module baru: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.mm-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ MM Module.
     */
    public function edit(FaqJasaKonstruksiMmModule $faq)
    {
        return view('admin.faq.faq-jasa-konstruksi.mm-module.edit', compact('faq'));
    }

    /**
     * Update FAQ MM Module yang sudah ada di database.
     */
    public function update(Request $request, FaqJasaKonstruksiMmModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-jasa-konstruksi/mm/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-jasa-konstruksi/mm'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ MM Module: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.mm-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ MM Module dari database.
     */
    public function destroy(FaqJasaKonstruksiMmModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ MM Module: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-konstruksi.mm-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}