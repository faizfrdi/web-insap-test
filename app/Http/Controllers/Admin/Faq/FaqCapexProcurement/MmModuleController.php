...
class MmModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqCapexProcurementMmModule::query();
        ...
        return view('admin.faq.faq-capex-procurement.mm-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-capex-procurement.mm-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-capex-procurement/mm'), $imageName); // Simpan gambar di folder public/images/faq-capex-procurement/mm
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ MM Module Capex Procurement: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.mm-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ MM Module.
     */
    public function edit(FaqCapexProcurementMmModule $faq)
    {
        return view('admin.faq.faq-capex-procurement.mm-module.edit', compact('faq'));
    }

    /**
     * Update FAQ MM Module yang sudah ada di database.
     */
    public function update(Request $request, FaqCapexProcurementMmModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-capex-procurement/mm/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-capex-procurement/mm'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ MM Module Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.mm-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ MM Module dari database.
     */
    public function destroy(FaqCapexProcurementMmModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ MM Module Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.mm-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}