...
class GeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqCapexProcurement::query();
        ...
        return view('admin.faq.faq-capex-procurement.general.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-capex-procurement.general.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-capex-procurement'), $imageName); // Simpan gambar di folder public/images/faq-capex-procurement/general
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ General Capex Procurement: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.general.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ General.
     */
    public function edit(FaqCapexProcurement $faq)
    {
        return view('admin.faq.faq-capex-procurement.general.edit', compact('faq'));
    }

    /**
     * Update FAQ General yang sudah ada di database.
     */
    public function update(Request $request, FaqCapexProcurement $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-capex-procurement/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-capex-procurement'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ General Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.general.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ General dari database.
     */
    public function destroy(FaqCapexProcurement $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ General Capex Procurement: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-capex-procurement.general.index')->with('success', 'FAQ berhasil dihapus.');
    }
}