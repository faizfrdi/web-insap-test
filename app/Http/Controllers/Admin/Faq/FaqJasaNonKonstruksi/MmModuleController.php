...
class MmModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqJasaNonKonstruksiMmModule::query();
        ...
        return view('admin.faq.faq-jasa-non-konstruksi.mm-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-jasa-non-konstruksi.mm-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-jasa-non-konstruksi/mm'), $imageName); // Simpan gambar di folder public/images/faq-jasa-non-konstruksi/mm
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ MM Module Jasa Non Konstruksi: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.mm-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ MM Module.
     */
    public function edit(FaqJasaNonKonstruksiMmModule $faq)
    {
        return view('admin.faq.faq-jasa-non-konstruksi.mm-module.edit', compact('faq'));
    }

    /**
     * Update FAQ MM Module yang sudah ada di database.
     */
    public function update(Request $request, FaqJasaNonKonstruksiMmModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-jasa-non-konstruksi/mm/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-jasa-non-konstruksi/mm'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ MM Module Jasa Non Konstruksi: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.mm-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ MM Module dari database.
     */
    public function destroy(FaqJasaNonKonstruksiMmModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ MM Module Jasa Non Konstruksi: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-jasa-non-konstruksi.mm-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}