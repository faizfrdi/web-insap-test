...
@section('title', 'Daftar Updates FICO/FM')
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="mb-0">Daftar Updates FICO/FM</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3 mobile-layout">
                <a href="{{ route('fico-fm.create') }}" class="btn btn-primary d-flex align-items-center shadow-sm" style="font-weight: 500; font-size: 0.9rem; padding: 0.5rem 1rem; border-radius: 0.5rem;">
                    <i class="bi bi-plus-circle me-2"></i> Tambah
                </a>
                <form class="search-form" method="GET" action="{{ route('fico-fm.index') }}">
            ...
            <div class="d-flex align-items-center mb-3">
                ...
                <form id="entriesPerPageForm" action="{{ route('fico-fm.index') }}" method="GET">
                ...
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($report->image)
                                    <img src="{{ asset('images/fico_fm/' . $report->image) }}" alt="Gambar Updates">
                                @else
                                    <span>-</span>
                                @endif
                            </td>
                            <td>
                                @if ($report->file)
                                    <a href="{{ asset('files/fico_fm/' . $report->file) }}" target="_blank" rel="noopener noreferrer">{{ basename($report->file) }}</a>
                                @else
                                    <span>-</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="d-flex flex-wrap gap-2">
                                    <a href="{{ route('fico-fm.public.show', $report->id) }}" class="btn btn-sm btn-primary" target="_blank" rel="noopener noreferrer"><i class="bi bi-eye"></i> Lihat</a>
                                    <a href="{{ route('fico-fm.edit', $report->id) }}" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i> Edit</a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form method="POST" action="{{ route('fico-fm.destroy', $report->id) }}">
                    @csrf
                    @method('DELETE')
