...
@section('title', 'Edit Jasa Konstruksi')
...
<div class="container mt-4">
    <div class="card card-custom">
        <div class="card-header-custom">
            <h4 class="mb-0">Edit Jasa Konstruksi</h4>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form id="editForm" action="{{ route('jasa-konstruksi.update', $jasa_konstruksi->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="judul" class="form-label">Judul Jasa Konstruksi</label>
                    <input type="text" class="form-control @error('judul') is-invalid @enderror" id="judul" name="judul" value="{{ old('judul', $jasa_konstruksi->judul) }}" required>
                    @error('judul')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('input_process') is-invalid @enderror" id="input_process" name="input_process" rows="3" required>{{ old('input_process', $jasa_konstruksi->input_process) }}</textarea>
                    @error('input_process')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('output_process') is-invalid @enderror" id="output_process" name="output_process" rows="3">{{ old('output_process', $jasa_konstruksi->output_process) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('pic') is-invalid @enderror" id="pic" name="pic" value="{{ old('pic', $jasa_konstruksi->pic) }}">
                    @error('pic')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('t_code') is-invalid @enderror" id="t_code" name="t_code" value="{{ old('t_code', $jasa_konstruksi->t_code) }}">
                    @error('t_code')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('proses') is-invalid @enderror" id="proses" name="proses" rows="3">{{ old('proses', $jasa_konstruksi->proses) }}</textarea>
                    @error('proses')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('link_terkait') is-invalid @enderror" id="link_terkait" name="link_terkait" value="{{ old('link_terkait', $jasa_konstruksi->link_terkait) }}">
                    @error('link_terkait')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    <label for="urutan" class="form-label">Urutan Jasa Konstruksi</label>
                    <input type="text" class="form-control @error('urutan') is-invalid @enderror" id="urutan" name="urutan" value="{{ old('urutan', $jasa_konstruksi->urutan) }}" required>
                    @error('urutan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('module') is-invalid @enderror" id="module" name="module" required>
                        ...
                        <option value="FI Module" {{ (old('module', $jasa_konstruksi->module) == 'FI Module') ? 'selected' : '' }}>FI Module</option>
                        <option value="SD Module" {{ (old('module', $jasa_konstruksi->module) == 'SD Module') ? 'selected' : '' }}>SD Module</option>
                        <option value="PS Module" {{ (old('module', $jasa_konstruksi->module) == 'PS Module') ? 'selected' : '' }}>PS Module</option>
                        <option value="CO/FM Module" {{ (old('module', $jasa_konstruksi->module) == 'CO/FM Module') ? 'selected' : '' }}>CO/FM Module</option>
                        <option value="MM Module" {{ (old('module', $jasa_konstruksi->module) == 'MM Module') ? 'selected' : '' }}>MM Module</option>
                    </select>
                    @error('module')
                        <div class="invalid-feedback">
                            {{ $message }}
                ...
                <!-- Bagian Gambar Utama -->
                <div class="mb-3">
                    <label for="image" class="form-label">Gambar Jasa Konstruksi</label>
                    <input type="file" class="form-control @error('image') is-invalid @enderror" id="image" name="image">
                    @if ($jasa_konstruksi->image)
                        <div class="text-center mt-3">
                            <p class="form-label text-center">Gambar Saat Ini:</p>
                            <img src="{{ asset('images/jasa_konstruksi/' . $jasa_konstruksi->image) }}" 
                                alt="{{ $jasa_konstruksi->judul }}" 
                                class="jasa-konstruksi-image img-fluid mx-auto">
                        </div>
                    @endif
                    @error('image')
                        <div class="invalid-feedback">
                ...
                <div class="d-flex mt-3">
                    <a href="{{ route('jasa-konstruksi.index') }}" class="btn btn-secondary me-2 btn-custom">Batal</a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmUpdateModalLabel">Konfirmasi Update Jasa Konstruksi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                Apakah Anda yakin ingin memperbarui data jasa konstruksi ini?
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const originalData = {
            judul: '{!! $jasa_konstruksi->judul !!}',
            input_process: '{!! $jasa_konstruksi->input_process !!}',
            output_process: '{!! $jasa_konstruksi->output_process !!}',
            pic: '{!! $jasa_konstruksi->pic !!}',
            t_code: '{!! $jasa_konstruksi->t_code !!}',
            proses: '{!! $jasa_konstruksi->proses !!}',
            link_terkait: '{!! $jasa_konstruksi->link_terkait !!}',
            urutan: '{!! $jasa_konstruksi->urutan !!}',
            module: '{!! $jasa_konstruksi->module !!}'
        };
