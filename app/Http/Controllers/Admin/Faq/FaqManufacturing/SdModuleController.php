...
class SdModuleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = FaqManufacturingSdModule::query();
        ...
        return view('admin.faq.faq-manufacturing.sd-module.index', compact('faqs', 'searchTerm'));
    }
    ...
    {
        return view('admin.faq.faq-manufacturing.sd-module.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image->move(public_path('images/faq-manufacturing/sd'), $imageName); // Simpan gambar di folder public/images/faq-manufacturing/sd
        }
        ...
        ActivityLog::create([
            'activity' => 'Admin menambahkan FAQ SD Module Manufacturing: ' . $request->input('question'),
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.sd-module.index')->with('success', 'FAQ berhasil ditambahkan.');
    }

    /**
     * Tampilkan form edit untuk FAQ SD Module.
     */
    public function edit(FaqManufacturingSdModule $faq)
    {
        return view('admin.faq.faq-manufacturing.sd-module.edit', compact('faq'));
    }

    /**
     * Update FAQ SD Module yang sudah ada di database.
     */
    public function update(Request $request, FaqManufacturingSdModule $faq)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($faq->image) {
                $oldImagePath = public_path('images/faq-manufacturing/sd/' . $faq->image);
            ...
            $extension = $image->getClientOriginalExtension();
            $imageName = $originalName . '_' . time() . '.' . $extension;
            $image->move(public_path('images/faq-manufacturing/sd'), $imageName);
            $faq->image = $imageName;
        }
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin memperbarui FAQ SD Module Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.sd-module.index')->with('success', 'FAQ berhasil diperbarui.');
    }

    /**
     * Hapus FAQ SD Module dari database.
     */
    public function destroy(FaqManufacturingSdModule $faq)
    {
        ...
        // Tambahkan log aktivitas
        ActivityLog::create([
            'activity' => 'Admin menghapus FAQ SD Module Manufacturing: ' . $faq->question,
            'admin_id' => auth()->user()->id,
        ]);

        return redirect()->route('faq-manufacturing.sd-module.index')->with('success', 'FAQ berhasil dihapus.');
    }
}