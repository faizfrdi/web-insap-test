...
<div class="container mt-4">
    <div class="card card-custom">
        <div class="card-header-custom">
            <h4 class="mb-0"><i class="fas fa-edit me-2"></i>Edit PS</h4>
        </div>
        <div class="card-body">
            @if ($errors->any())
            ...
            <form id="editForm" action="{{ route('ps.update', $ps->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label for="report" class="form-label">Updates</label>
                    <textarea class="form-control @error('report') is-invalid @enderror" id="report" name="report" rows="3">{{ old('report', $ps->report) }}</textarea>
                    @error('report')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                        ...
                        <option value="done" {{ old('status', $ps->status) == 'done' ? 'selected' : '' }}>Done</option>
                        <option value="on going" {{ old('status', $ps->status) == 'on going' ? 'selected' : '' }}>On Going</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="4">{{ old('description', $ps->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="link" id="link" class="form-control" rows="5">{{ old('link', $ps->link) }}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    @if($ps->image)
                        <div class="text-center mt-3">
                            ...
                            <img src="{{ asset('images/ps/' . $ps->image) }}" alt="Current Image" class="jasa-konstruksi-image img-fluid mx-auto">
                        </div>
                    @endif
                ...
                <div class="mb-3">
                    ...
                    @if($ps->file)
                        <a href="{{ asset('files/ps/' . $ps->file) }}" target="_blank" class="d-block mt-2">Lihat File</a>
                    @endif
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="confirmUpdateModalLabel">Konfirmasi Update PS</h5>
